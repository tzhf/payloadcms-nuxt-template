...
import type { Image, Page, TextBlock } from '#payload-types'
...
interface Props {
  ...
  content?: LexicalRichTextField | null
  linkClass?: string
...
const richTextContent = computed(() => {
  ...
  const rawContent = props.content ?? props.block?.content
  if (rawContent && 'root' in rawContent && rawContent.root?.children) {
    return rawContent.root.children
  }
...
const getElementAttrs = (node: LexicalElementNode) => {
  ...
  if ('format' in node && node.format) {
    ...
    if (alignClass) attrs.class = alignClass
  }
